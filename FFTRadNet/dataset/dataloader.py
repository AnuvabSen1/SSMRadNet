...
def RADIal_collate(batch):
    ...
    for radar_FFT, segmap,out_label,box_labels,image in batch:
        ...
        FFTs.append(torch.from_numpy(radar_FFT).to(torch.float32))
        segmaps.append(torch.from_numpy(segmap).to(torch.float32))
        encoded_label.append(torch.from_numpy(out_label).to(torch.float32))
        images.append(torch.from_numpy(image).to(torch.float32))
...
def RADIal_collate_without_image(batch):
    FFTs = []
    segmaps = []
    labels = []
    encoded_label = []
...
def CreateDataLoaders_without_images(dataset,config=None,seed=0):
    ...
    if(config['mode']=='random'):
        ...
        train_loader = DataLoader(train_dataset, 
                                ...
                                num_workers=config['train']['num_workers'],
                                pin_memory=True,
                                collate_fn=RADIal_collate_without_image,

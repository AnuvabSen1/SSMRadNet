...
class FFTRadNet(nn.Module):
    ...
    def forward(self,x):
        ...
        # print(f"Detection shape {out['Detection'].shape}, Segmentation shape {out['Segmentation'].shape}")
        # Detection shape torch.Size([1, 3, 128, 224]), Segmentation shape torch.Size([1, 1, 256, 224])
        return out
...
if __name__ == "__main__":
    ...
    net = FFTRadNet(blocks = config['model']['backbone_block'],
                        ...
                        segmentation_head = config['model']['SegmentationHead'])#.to("cuda")
    
    net.eval()


    x = torch.rand(1, 32, 512, 256)#.to("cuda")
    ...
    print("Done FFTRadNet " + str(test_iterations) + "iterations")

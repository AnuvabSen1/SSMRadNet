...
def GetFullMetrics(predictions,object_labels,range_min=5,range_max=100,IOU_threshold=0.3, verbose=True):
    perfs = {}
    precision = []
    recall = []
    iou_threshold = []
    ...
    scores = {'mAP': np.mean(perfs['precision']),
            'mAR': np.mean(perfs['recall']),
            'F1': F1_score,
            'RE': np.mean(RangeError),
            'AE': np.mean(AngleError)}
